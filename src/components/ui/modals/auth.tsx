          ...
          <TabsContent value="auth" className="pt-4">
            <form onSubmit={loginForm.handleSubmit(loginSubmit)} className="flex flex-col gap-4">
              <div className="grid w-full max-w-sm items-center gap-1.5">
                ...
                <Input {...loginForm.register('nickname', { required: true })} type="login" id="login"
                       placeholder="NicknameIsAvailable" />
              </div>
              <div className="grid w-full max-w-sm items-center gap-1.5">
                <Label htmlFor="email">Пароль</Label>
                <Input {...loginForm.register('password', { required: true })} type="password" id="password"
                       placeholder="password123" />
              </div>
              <WalletOptions disabled={!loginForm.formState.isValid && !registrationForm.formState.isValid}
                             type="submit" />
            </form>
          </TabsContent>
          <TabsContent value="registration" className="pt-4">
            <form onSubmit={registrationForm.handleSubmit(registrationSubmit)} className="flex flex-col gap-4">
              <div className="grid w-full max-w-sm items-center gap-1.5">
                <Label htmlFor="email">Твой никнейм</Label>
                <Input {...registrationForm.register('nickname', { required: true })} type="login" id="login"
                       placeholder="NicknameIsAvailable" />
              </div>
              <div className="grid w-full max-w-sm items-center gap-1.5">
                <Label htmlFor="email">Пароль</Label>
                <Input {...registrationForm.register('password', { required: true })} type="password" id="password"
                       placeholder="password123" />
              </div>
              <div className="grid w-full max-w-sm items-center gap-1.5">
                <Label htmlFor="email">Подтвердите пароль</Label>
                <Input {...registrationForm.register('repeatPassword', { required: true })} type="password"
                       id="password" placeholder="password123" />
              </div>
              <WalletOptions disabled={!loginForm.formState.isValid && !registrationForm.formState.isValid}
                             type="submit" />
            </form>
